...
int zb_ble_hci_cmd_handler(u16 clusterId, u8 len, u8 *payload){
	...
	if(clusterId == APP_BLE_CMD_ZB_NWK_MANAGEMENT_SET){
		...
		if(g_joinNetworkInfo.timerSteering){
			TL_ZB_TIMER_CANCEL(&g_joinNetworkInfo.timerSteering);
		}
		...
		g_joinNetworkInfo.timerSteering = TL_ZB_TIMER_SCHEDULE(sampleLight_bdbNetworkJoinDirect, NULL, 50);
	}else{
		ret = -1;
	}
	return ret;
