...
void zbdemo_bdbCommissioningCb(u8 status, void *arg){
	...
	switch(status){
		case BDB_COMMISSION_STA_SUCCESS:
			...
			g_zcl_onOffAttrs_2.startUpOnOff = ZCL_START_UP_ONOFF_SET_ONOFF_TO_PREVIOUS;
#endif
#endif

			light_blink_start(SAMPLE_LIGHT_ENDPOINT, 2, 200, 200);
...
void sampleLight_leaveCnfHandler(nlme_leave_cnf_t *pLeaveCnf)
{
    if(pLeaveCnf->status == SUCCESS){
    	...
    	light_blink_start(SAMPLE_LIGHT_ENDPOINT, 3, 200, 200);
