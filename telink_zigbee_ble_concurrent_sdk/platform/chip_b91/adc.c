...
#define ADC_DBG_EN 0
#if ADC_DBG_EN
volatile unsigned short T_adcV[128] = {0};
int adcCnt = 0;
#endif

unsigned short adc_sample_and_get_result(void){
	unsigned short  v = adc_get_code();
	v = (((v * g_adc_vbat_divider * g_adc_pre_scale * g_adc_vref)>>13) + g_adc_vref_offset);

#if ADC_DBG_EN
	T_adcV[(adcCnt++)&0x7f] = v;
#endif
